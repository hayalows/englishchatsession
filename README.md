...
- Optionally records privacy-respecting first-party finder opens, one scan-start action per user request, and coarse visible-time engagement milestones; the private `/analytics` report supports rolling windows, audience filters, and repeat-scan diagnostics without recording individual calendar checks.
